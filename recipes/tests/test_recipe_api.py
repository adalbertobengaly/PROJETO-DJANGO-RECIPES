...
class RecipeAPIv2TestMixin(RecipeMixin):
    ...
    def get_recipe_raw_data(self):
        return {
            "title": "Miojo!",
            "description": "Miojo de qualquer sabor!",
            "servings": 6,
            "preparation_time": 10,
            "preparation_time_unit": "Minutos",
            "servings_unit": "Pessoas",
            "preparation_steps": "Modo de preparo é só tacar na água e deixar ferver até chegar ao ponto desejado e tacar o tempero (sachê)."
        }
...
class RecipeAPIv2Test(test.APITestCase, RecipeAPIv2TestMixin):
    ...
    def test_recipe_api_list_loads_recipes_by_category_id(self):
        ...
        # Creates 10 recipes
        recipes = self.make_recipes_in_batch(qtd=10)
    ...
    def test_recipe_api_list_logged_user_cant_update_a_recipe_owned_by_another_user(self):
        # Arrange (config do test)
        recipe = self.make_recipe()
        user_data = self.get_auth_data(username='test_patch')

        # This user cannot update the recipe beacuse it is owned by 
        # another user.
        another_user = self.get_auth_data(username='cant_update')
        jwt_access_token_another_user = another_user.get('jwt_access_token')

        # This is the actual owner of the recipe
        author = user_data.get('user')
        recipe.author = author
        recipe.save()
        ...
        response = self.client.patch(
            reverse('recipes:recipes-api-detail', args=(recipe.id,)),
            HTTP_AUTHORIZATION=f'Bearer {jwt_access_token_another_user}'
        )

        # Assertion (Afirmação)
        # Another user cannot update the recipe, so the status code must be 403 
        # Forbidden. The server understood the request but refuses 
        # to authorize it
        self.assertEqual(
            response.status_code,
            403
